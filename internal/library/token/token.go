...
// Login 登录
func Login(ctx context.Context, user *model.ContextUser) (string, int64, error) {
	claims := Claims{
		user,
		jwt.RegisteredClaims{},
	}
	...
	var (
		...
		// 身份绑定
		bindKey = GetBindKey(user.App, gconv.Int64(user.Id))
		// 有效时长
...
func Logout(r *ghttp.Request) (err error) {
	...
	var (
		...
		// 身份绑定
		bindKey = GetBindKey(claims.App, gconv.Int64(claims.Id))
	)
...
func ParseLoginUser(r *ghttp.Request) (user *model.ContextUser, err error) {
	var (
		ctx    = r.Context()
		header = GetAuthorization(r)
	)
...
func GetAuthKey(token string) string {
	return gmd5.MustEncryptString("nineStar" + token)
}
